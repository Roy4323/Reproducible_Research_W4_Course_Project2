library(dplyr)
data_set<-download.file("https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2FStormData.csv.bz2","/cloud/project/storm_dataset")

StormData <- read.csv(bzfile("storm_dataset"))
head(data_set)
...
propdmg <- aggregate(PROPDMG ~ EVTYPE, data = data_set, FUN = sum)
...
cropdmg <- aggregate(CROPDMG ~ EVTYPE, data = data_set, FUN = sum)
